...
ISSUE_STATES = {"todo", "in_progress", "blocked", "done", "skipped"}
...
def _validate_issue_scope(payload: dict[str, Any]) -> dict[str, Any]:
    issue_id = str(payload.get("issue_id", "")).strip()
    issue_state = str(payload.get("issue_state", "")).strip()
    if not issue_id:
        raise ValueError("status payload requires non-empty issue_id")
    if issue_state not in ISSUE_STATES:
        raise ValueError(f"status payload requires issue_state in {sorted(ISSUE_STATES)}")
...
def read_status(status_path: str | Path) -> dict[str, Any]:
    ...
    return _validate_issue_scope(json.loads(path.read_text(encoding="utf-8")))
...
def write_status(
    ...
    *,
    ...
    return _write_payload(Path(status_path), _validate_issue_scope(payload))
...
def update_status(
    ...
    *,
    ...
    return _write_payload(path, _validate_issue_scope(payload))
